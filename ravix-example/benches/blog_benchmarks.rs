...
use ravix_example::{db, models::post::CreatePostDto, models::user::CreateUserDto};
use std::sync::Arc;
...
async fn build_test_app(mongo_uri: &str) -> axum::Router {
    ...
    ravix::router::RouterBuilder::build(container_ref)
}
...
fn bench_auth_login(c: &mut Criterion) {
    ...
    c.bench_function("auth_login", |b| {
        b.to_async(&rt).iter(|| async {
            let login_dto = ravix_example::models::user::LoginDto {
                email: "bench_login@example.com".to_string(),
                password: "password123".to_string(),
            };
            let _ = app
