...
pub fn expand(_args: TokenStream, input: TokenStream) -> TokenStream {
                ...
                match opts {
                    InjectOptions::Required => {
                        ...
                        binding_check_submissions.push(quote! {
                            ::rusta_di::inventory::submit!(::rusta_di::BindingCheck {
                                type_name: #type_name_str,
                                check: |c: &::rusta_di::Container| {
                                    if c.has_binding::<#field_ty2>() {
                                        ::std::result::Result::Ok(())
                                    } else {
                                        ::std::result::Result::Err(
                                            ::std::format!(
                                                "[rusta-di] Required binding missing for `{}`",
                                                #type_name_str,
                                            )
                                        )
                                    }
    ...
    let expanded = quote! {
        ...
        impl ::rusta_di::Injectable for #struct_name {
            fn construct(container: &::rusta_di::Container) -> ::std::sync::Arc<Self> {
