...
impl Container {
    ...
    pub fn resolve<T: Clone + Send + Sync + 'static>(&self) -> T {
        ...
        self.singletons
            ...
            .unwrap_or_else(|| {
                panic!(
                    "[ravix-di] No binding registered for `{}`. \
            ...
            .unwrap_or_else(|| {
                panic!(
                    "[ravix-di] Type mismatch resolving `{}`. This is a bug in the DI container.",
                    type_name::<T>()
                )
            })
    }
    ...
    pub fn resolve_named<T: Clone + Send + Sync + 'static>(&self, name: &'static str) -> T {
        ...
        self.named_singletons
            ...
            .unwrap_or_else(|| {
                panic!(
                    "[ravix-di] No named binding '{}' registered for `{}`.",
                    name,
                    type_name::<T>()
                )
            })
            ...
            .unwrap_or_else(|| {
                panic!(
                    "[ravix-di] Type mismatch resolving named '{}' for `{}`.",
                    name,
                    type_name::<T>()
                )
            })
